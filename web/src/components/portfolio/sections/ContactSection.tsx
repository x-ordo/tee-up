      ...
      <div className="mx-auto max-w-4xl">
        <Card className="overflow-hidden border-2 border-tee-accent-primary/20 bg-gradient-to-br from-tee-surface to-tee-background p-12 text-center">
          ...
          {showForm || (!hasExternalLinks && proId && showRequestForm) ? (
            ...
            <>
              {/* Primary CTA - KakaoTalk (largest, most prominent) */}
              {openChatUrl && (
                <div className="mb-4">
                  <Button
                    asChild
                    size="lg"
                    className="h-14 gap-3 bg-tee-kakao px-8 text-lg font-semibold text-tee-kakao-text shadow-lg hover:bg-tee-kakao/90"
                  >
              ...
              {/* Secondary CTAs - smaller, outline style */}
              <div className="flex flex-col items-center justify-center gap-3 sm:flex-row">
                ...
                {/* Fallback contact button - Primary when no KakaoTalk */}
                {!hasExternalLinks && onContactClick && (
                  <Button size="lg" onClick={onContactClick} className="h-14 px-8 text-lg font-semibold shadow-lg">
                    레슨 문의하기
                  </Button>
                )}
              </div>
