...
**Base Neutrals (90%)**
```css
--tee-background: #F7F4F0  /* 페이지 배경 */
--tee-surface: #FFFFFF     /* 카드 배경, 호버 */
--tee-stone: #E8E8E5       /* 테두리, 구분선 */
--tee-ink-muted: #5E5E5A   /* 비활성 텍스트, 레이블 */
--tee-ink-light: #52524E   /* 본문 텍스트 */
--tee-ink-strong: #1A1A1A  /* 헤딩, 강조 */
```

**Accent Primary (Deep Green)**
```css
--tee-accent-primary: #0A362B           /* 프라이머리 액션 */
--tee-accent-primary-hover: #072A21     /* 호버 */
--tee-accent-primary-active: #051E18    /* 액티브 */
--tee-accent-primary-disabled: #B4C6BF  /* 비활성 */
--tee-accent-secondary: #B39A68         /* 프리미엄 포인트 */
```

**Functional Colors (2%)**
```css
--tee-success: #388E3C  /* 성공 상태 */
--tee-warning: #FBC02D  /* 경고 상태 */
--tee-error: #D32F2F    /* 에러 상태 */
--tee-info: #1976D2     /* 정보 상태 */
```
...
| 요소 | 색상 | 이유 |
|------|------|------|
| 페이지 배경 | tee-background | 눈의 피로 감소 |
| 카드 배경 | tee-surface | 계층 구분 |
| 테이블 헤더 | tee-background | 시각적 분리 |
| 본문 텍스트 | tee-ink-light | 가독성과 부드러움 균형 |
| 헤딩 | tee-ink-strong | 명확한 시각적 계층 |
| 프라이머리 CTA | tee-accent-primary | 주목성과 신뢰감 |
| 프리미엄 강조 | tee-accent-secondary | 고급 포인트 |
| 링크 | tee-accent-primary | 일관된 인터랙티브 요소 |
...
.card:hover {
  border-color: var(--tee-accent-primary);
...
**구조:**
```
┌─ Table Header (tee-background 배경, uppercase) ─┐
├─ Row 1 (hover: tee-background)                ─┤
├─ Row 2                                       ─┤
├─ Row 3                                       ─┤
└───────────────────────────────────────────────┘
```
...
```css
.input {
  border: 2px solid var(--tee-stone);
  border-radius: 12px;
}

.input:focus {
  border-color: var(--tee-accent-primary);
  ring: 4px var(--tee-accent-primary-hover);
}
```
...
```css
/* Colors */
--tee-background, --tee-surface, --tee-stone
--tee-ink-strong, --tee-ink-light, --tee-ink-muted
--tee-accent-primary, --tee-accent-primary-hover, --tee-accent-primary-active
--tee-accent-primary-disabled, --tee-accent-secondary
--tee-success, --tee-warning, --tee-error, --tee-info
...
```javascript
// tailwind.config.ts colors (token-based)
colors: {
